...
export const signupasStudent = async (req, res, next) => {
  ...
  try {
    ...
    if (validUser) return next(errorHandler(404, 'Email already taken'));
    await newUser.save();
...
export const signupasLandlord = async (req, res, next) => {
  ...
  const newUser = new User({ 
    ...
    phoneno,
    password: hashedPassword,
    userType: 'landlord', // Set userType to 'landlord'
  });
  try {
    const validUser = await User.findOne({ email });
    if (validUser) return next(errorHandler(404, 'Email already taken'));
